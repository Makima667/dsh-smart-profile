...
async function main() {
  ...
  if (command === 'compat') {
    const target = args[1] && !args[1].startsWith('-') ? args[1] : 'next'
    return console.log(JSON.stringify(await compatibilityReport(target), null, 2))
  }
  ...
  if (command === 'plan' || command === 'apply') {
    const spec = args[1]
    if (!spec || spec.startsWith('-')) throw new Error('Package name is required.')
    const at = spec.lastIndexOf('@'), scoped = spec.startsWith('@'), splitVersion = at > (scoped ? spec.indexOf('/') : 0)
    const candidate = { name: splitVersion ? spec.slice(0, at) : spec, version: splitVersion ? spec.slice(at + 1) : null, metadata: { dsh: { bundle: { patch: 'declared-by-user-plan' } } }, riskFlags: [] }
    const profile = argValue(args, '--profile', 'web')
    if (command === 'plan') return console.log(JSON.stringify(buildInstallPlan(candidate, { profile }), null, 2))
    return console.log(JSON.stringify(await safeInstall(candidate, { profile, approved: args.includes('--approve'), allowRisky: args.includes('--allow-risky'), allowUndeclaredBundle: true }), null, 2))
  }
