...
async function main() {
  ...
  if (command === 'plan' || command === 'apply') {
    ...
    const splitVersion = at > (scoped ? spec.indexOf('/') : 0)
    ...
    const result = await safeInstall(candidate, { profile, approved: args.includes('--approve'), allowRisky: args.includes('--allow-risky'), allowUndeclaredBundle: true })
