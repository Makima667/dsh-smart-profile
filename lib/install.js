...
export function buildInstallPlan(candidate, { profile = 'web', dshSpec = DEFAULT_DSH } = {}) {
  ...
  return {
    ...
    profile,
    commands: {
      snapshot: [npxCommand(), ['--yes', dshSpec, '--profile', profile, '--dump-config']],
      install: [npxCommand(), ['--yes', dshSpec, 'plugin', '--profile', profile, 'add', packageSpec]],
      verify: [npxCommand(), ['--yes', dshSpec, '--profile', profile, '--dump-config']],
      rollback: [npxCommand(), ['--yes', dshSpec, 'plugin', '--profile', profile, 'remove', candidate.name]],
    },
  }
}
...
export function defaultRunner(command, args) {
  return new Promise((resolve, reject) => {
    ...
    child.on('exit', (code, signal) => signal ? reject(new Error(`Command terminated by ${signal}`)) : resolve({ code: code ?? 1, stdout, stderr }))
  })
}
...
export async function safeInstall(candidate, { profile = 'web', dshSpec = DEFAULT_DSH, approved = false, allowRisky = false, allowUndeclaredBundle = false, runner = defaultRunner } = {}) {
  ...
  let before = null
  try { before = await runner(...plan.commands.snapshot) } catch { /* new profiles may not exist yet */ }
  const install = await runner(...plan.commands.install)
  if (install.code !== 0) {
    let rollback = null
    try { rollback = await runner(...plan.commands.rollback) } catch { /* best effort */ }
    return { status: 'install-failed', plan, install: { code: install.code, stderr: install.stderr }, rollback: rollback ? { code: rollback.code } : null, beforeHash: hashText(before?.stdout) }
  }
  ...
  if (!verified) {
    let rollback
    try { rollback = await runner(...plan.commands.rollback) } catch (error) { rollback = { code: 1, stderr: error.message } }
    return { status: 'verification-failed-rolled-back', plan, verify: { code: verify.code, stderr: verify.stderr }, rollback: { code: rollback.code, stderr: rollback.stderr }, beforeHash: hashText(before?.stdout), afterHash: hashText(verify.stdout) }
  }
